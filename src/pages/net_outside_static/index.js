...
export default class NetOutsideStatic extends Component{
    constructor(props){
        super(props);
        this.state={
            ipaddr:null,
    ...
    render(){
        ...
        let {ipaddr,netmask,gateway,dns0,dns1,mtu,macaddr,check_dns0,check_dns1,check_gateway,check_ipaddr,check_macaddr,check_mtu,check_netmask} = this.state;
        // 无关页面不显示默认信息
        let infosx;
        if(proto==='static'){
            infosx = infos;
        ...
        return(
            ...
            <form onSubmit={(event)=>this.handleSubmit(event)}>
                <div className="input-group">
                    <label>IP地址：</label>
                    <input required name='ipaddr' ref='ipaddr' onChange={(e)=>this.handleIpaddrChange(e.target.value)} value={ipaddr!==null?ipaddr:infosx.ipaddr} type="text" />
                    <span className='icon'>*</span>
                    <p style={{display:check_ipaddr?'block':'none'}} className='alert'>IP地址格式错误</p>
                </div>
                <div className="input-group">
                    <label>子网掩码：</label>
                    <input required name='netmask' ref='netmask' onChange={(e)=>this.handleNetmaskChange(e.target.value)} value={netmask!==null?netmask:infosx.netmask} type="text" />
                    <span className='icon'>*</span>
                    <p style={{display:check_netmask?'block':'none'}} className='alert'>子网掩码格式错误</p>
                </div>
                <div className="input-group">
                    <label>网关：</label>
                    <input required name='gateway' ref='gateway' onChange={(e)=>this.handleGatewayChange(e.target.value)} value={gateway!==null?gateway:infosx.gateway} type="text" />
                    <span className='icon'>*</span>
                    <p style={{display:check_gateway?'block':'none'}} className='alert'>网关格式错误</p>
                </div>
                <div className="input-group">
                    <label>自定义首选DNS：</label>
                    <input required name='dns0' ref='dns0' onChange={(e)=>this.handledns0Change(e.target.value)} value={dns0!==null?dns0:infosx.dns?infosx.dns.length>0?infosx.dns[0]:'':''} type="text" />
                    <span className='icon'>*</span>                    
                ...
                <div className="input-group">
                    <label>设置MTU：</label>
                    <input name='mtu' ref='mtu' onChange={(e)=>this.handleMtuChange(e.target.value)} value={mtu!==null?mtu:infosx.mtu} type="text" />
    ...
    handleSubmit(e){
        ...
        gateway = gateway!==null?gateway:this.refs.gateway.value;
        ...
        check_gateway = !global.exp.ip.test(gateway);
        check_dns0 = !global.exp.ip.test(dns0);
        ...
        // console.log(check_ipaddr,check_netmask,check_gateway,check_dns0,check_dns1,check_macaddr,check_mtu);
        if(check_ipaddr||check_netmask||check_gateway||check_dns0||check_dns1||check_macaddr||check_mtu){
